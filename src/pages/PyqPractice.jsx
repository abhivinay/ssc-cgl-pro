...
const initAttempts = (value) =>
  value && typeof value === "object" && !Array.isArray(value) ? value : {};
const initPosition = (value) => ({
  subject: typeof value?.subject === "string" ? value.subject : "",
  year: typeof value?.year === "string" ? value.year : "",
  topic: typeof value?.topic === "string" ? value.topic : "",
  index:
    Number.isSafeInteger(value?.index) && value.index >= 0 ? value.index : 0,
});
...
export default function PyqPractice() {
  ...
  const setSubject = (v) => setPosition((p) => ({ ...p, subject: v })),
    setYear = (v) => setPosition((p) => ({ ...p, year: v })),
    setTopic = (v) => setPosition((p) => ({ ...p, topic: v })),
    setIndex = (v) =>
      setPosition((p) => ({
        ...p,
        index: typeof v === "function" ? v(p.index) : v,
      }));
  useEffect(() => {
    const controller = new AbortController();
    fetch(`${base}questions.json`, { signal: controller.signal })
      .then((r) => {
        if (!r.ok) throw Error("Question bank could not load. Please retry.");
        return r.json();
      })
      .then(setRows)
      .catch((e) => {
  ...
  const subjects = useMemo(
    () => [...new Set(rows.map((q) => q.subject))].sort(),
    [rows],
  );
    ...
    () =>
      rows.filter(
        (q) =>
          ...
          (!year || String(q.source.year) === year) &&
          (!topic || q.topic === topic),
      ),
    ...
    <div className="pyq-workspace">
      <header>
        ...
        <p className="mt-2 text-zinc-400">
          2019–2025 · 20,600 questions · 248 HOLD questions excluded from
          practice.
        </p>
        <p className="text-sm text-zinc-400">
          Source verification is incomplete. Questions requiring unavailable
      ...
      <div className="pyq-filters">
        {[
          ...
          <label key={label}>
            {label}
            <select
              className="mt-1 w-full rounded-xl bg-zinc-900 p-3"
              value={value}
      ...
      <p className="pyq-summary">
        {Object.keys(attempts).length} answered ·{" "}
        {Object.values(attempts).filter((a) => a.correct).length} correct ·
        Answers and position saved on this device
      </p>
      {error ? (
        <div role="alert">
          {error}
        ...
        <p role="status">Loading question bank…</p>
      ) : !q ? (
        ...
        <article className="pyq-question space-y-4">
          ...
          {q.images
            ...
            .map((p) => (
              <img
                className="max-h-80 max-w-full rounded bg-white object-contain"
                key={p}
                src={base + p}
                alt="Question figure"
          ...
          {blocked && (
            <p role="status" className="text-cyan-200">
              Required visual content is unavailable or needs review. Skip this
              question until its assets are recovered.
            </p>
          )}
          <fieldset disabled={blocked || shown} className="space-y-3">
            <legend className="sr-only">Choose your answer</legend>
            {q.options.map((o, i) => {
              const letter = "ABCD"[i];
              return (
                <label
                  className={`flex items-start gap-3 rounded-xl border p-4 ${selected === letter ? "border-cyan-300" : "border-zinc-700"}`}
                  key={letter}
                >
                  <input
                    type="radio"
                  ...
                  <span>
                    {letter}.{" "}
                    {q.optionImages[letter] ? (
                      <img
                        className="max-h-40 max-w-full bg-white"
                        src={base + q.optionImages[letter]}
                        alt={`Option ${letter}`}
                        onError={() => setImageFailed(true)}
                      />
                    ) : (
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  setError(
                    "Unable to save your answer. Open Data & recovery to check storage.",
                  );
                }
              }}
            >
          ...
          <nav aria-label="Question navigation">
            ...
            <button
              disabled={safeIndex + 1 >= filtered.length}
              className="p-3 disabled:opacity-40"
              onClick={() => move(1)}
            >
              Next question

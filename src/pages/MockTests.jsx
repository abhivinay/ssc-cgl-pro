...
export default function MockTests() {
  ...
  const saveMock = () => {
    if (
      !form.name.trim() ||
      ["score", "total", "accuracy", "attempted", "time"].some(
        (key) => form[key] === "" || !Number.isFinite(Number(form[key])),
      ) ||
      Number(form.total) <= 0 ||
      Number(form.score) > Number(form.total) ||
      Number(form.accuracy) < 0 ||
      Number(form.accuracy) > 100 ||
      Number(form.attempted) < 0 ||
      !Number.isInteger(Number(form.attempted)) ||
      Number(form.time) <= 0
    ) {
      setError(
        "Enter a name and valid numbers: positive total/time, score no higher than total, accuracy 0–100 and whole-number attempts.",
      );
      return;
    }
    setError("");
  ...
  return (
    <div className="mx-auto max-w-7xl pb-10">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Mock Analysis Center</h1>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
          ...
          {error && (
            <p role="alert" className="mt-3 text-cyan-200">
              {error}
            </p>
          )}
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, attempted: e.target.value })
                }
                className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 text-white"
                aria-label="Attempts"
              />
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, weakestSubject: e.target.value })
                }
                className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 text-white"
                aria-label="Weakest subject"
              >
            ...
            <button onClick={saveMock} className="primary-btn w-full">
              Save Mock
            </button>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
          <h2 className="text-2xl font-semibold">AI Mock Review</h2>
      ...
      <div className="mt-6 rounded-lg border border-zinc-800 bg-zinc-900 p-6">
        <h2 className="text-2xl font-semibold">Mock History</h2>

        {mocks.length === 0 ? (
          <p className="mt-6 text-zinc-400">No mock results saved yet.</p>
        ) : (

...
export default function GeminiExtractor() {
  ...
  async function extractPDF() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || `Extraction failed with status ${response.status}`,
        );
      }
        ...
        normalizeReviewQuestion(
          {
            ...
            correctAnswer:
              ...
              question.answer,
            source: question.source?.fileName ?? question.source ?? file.name,
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Upload an SSC question-paper PDF. Extracted questions will
          automatically appear in Review Center.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-6">
        ...
        {!file ? (
          <button
            ...
            className="flex min-h-40 w-full flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-700 bg-zinc-950/60 px-6 transition hover:border-cyan-300/40 hover:bg-cyan-300/10"
          >
            <div className="rounded-lg bg-cyan-300/10 p-4 text-cyan-200">
              {"Upload Cloud"}
            </div>
          ...
          <div className="space-y-5">
            <div className="flex flex-col gap-4 rounded-lg border border-zinc-800 bg-zinc-950/70 p-5 sm:flex-row sm:items-center">
              ...
              <button
                ...
                className="rounded-xl border border-zinc-700 p-2.5 text-zinc-400 transition hover:border-cyan-300/40 hover:text-cyan-200 disabled:cursor-not-allowed disabled:opacity-40"
            ...
            <button
              ...
              onClick={extractPDF}
              disabled={state === "extracting"}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-900 px-5 py-3 font-semibold text-white transition hover:bg-cyan-900 disabled:cursor-wait disabled:opacity-70"
            >
      ...
      {result && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Stat
            label="Paper"
            value={result.paper?.title || file?.name || "SSC PDF"}
          />
          <Stat label="Extracted" value={result.questionCount} />
          <Stat
            label="Review Center Total"
            value={result.totalReviewQuestions}
          />
...
function mergeQuestions(existing, incoming) {
  const signatures = new Set(
    existing.map((question) => questionSignature(question)),
  );

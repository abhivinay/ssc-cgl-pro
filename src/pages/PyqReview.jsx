  ...
  return {
    ...
    needsReview:
      question.needsReview !== undefined
        ...
        : !question.verified,
    validationErrors: Array.isArray(question.validationErrors)
...
export default function PyqReview() {
  ...
  const filteredQuestions = useMemo(() => {
    if (filter === "review")
      return questions.filter((question) => question.needsReview);
    if (filter === "verified")
      return questions.filter((question) => question.verified);
  ...
  function applyExtraction(questionId, extracted) {
    ...
    updateQuestionsAndSave((previous) =>
      previous.map((question) =>
    ...
    try {
      while (extractionQueue.isRunning()) {
        ...
        try {
          ...
          addLog(
            `Question ${item.questionNo || item.id} extracted successfully`,
            "success",
          );
  ...
  async function extractWithGemini() {
    ...
    try {
      ...
      addLog(
        `Question ${currentQuestion.questionNo || currentQuestion.id} extracted manually`,
        "success",
      );
      showMessage(
        `Question ${currentQuestion.questionNo || currentQuestion.id} extracted successfully`,
      );
    } catch (error) {
  ...
  function saveProgress() {
    ...
    if (validation.needsReview) {
      ...
      showMessage(
        `Saved with ${validation.errors.length} validation issue${validation.errors.length === 1 ? "" : "s"}`,
      );
    } else {
  ...
  function nextQuestion() {
    setCurrentIndex((index) =>
      Math.min(filteredQuestions.length - 1, index + 1),
    );
  }
  ...
  if (!questions.length) {
    return (
      <div className="rounded-lg border border-cyan-300/30 bg-cyan-300/10 p-6 text-cyan-200">
        <h1 className="text-xl font-semibold">
          PYQ dataset could not be loaded
        </h1>
        <p className="mt-2 text-sm">{message || `Missing file: ${DATA_URL}`}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 rounded-lg border border-zinc-800 bg-zinc-900 p-5 xl:flex-row xl:items-center xl:justify-between">
        <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-lg bg-cyan-900 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-900"
          >
            Download JSON
          </button>
        </div>
              ...
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">
                    ...
                    {currentQuestion.subject || "Unknown subject"}
                  </p>
                </div>
          ...
          {currentQuestion && (
            <div className="space-y-4">
              <button
                ...
                className="w-full rounded-xl bg-cyan-900 px-4 py-3 text-sm font-semibold text-white hover:bg-cyan-900 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {extracting
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateCurrentQuestion("questionText", event.target.value)
                  }
                  rows={7}
                  className="w-full rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-sm text-white outline-none focus:border-cyan-300/40"
                />
              </div>
                ...
                {currentQuestion.options.map((option, index) => (
                  <div key={index} className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-white outline-none focus:border-cyan-300/40"
                    />
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateCurrentQuestion("correctAnswer", event.target.value)
                  }
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  className="rounded-lg bg-cyan-900 px-4 py-3 text-sm font-semibold text-white hover:bg-cyan-900 disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      <div className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900 p-4">
        ...
        <p className="text-sm text-zinc-400">
          {message ||
            `${questions.filter((question) => question.verified).length} of ${questions.length} verified`}
        </p>
        ...
        <button
          ...
          className="rounded-lg bg-cyan-900 px-5 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"
        >
          Next
        </button>
      </div>

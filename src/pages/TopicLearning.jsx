...
const getStageXP = (stageId) => {
  ...
  if (
    ["conceptCheck", "level-1", "level-2", "level-3", "topic-test"].includes(
      stageId,
    )
  ) {
    return XP.PRACTICE;
...
const getStageQuestions = (content, stageId) => {
  const map = {
    conceptCheck: ["conceptCheck", "conceptQuestions", "conceptCheckQuestions"],

    "level-1": ["level1", "level1Questions", "basicQuestions"],

    "level-2": ["level2", "level2Questions", "intermediateQuestions"],

    "level-3": ["level3", "level3Questions", "advancedQuestions"],

    "topic-test": ["topicTest", "topicTestQuestions", "testQuestions"],

    pyq: ["pyq", "pyqQuestions", "previousYearQuestions"],
  };
  ...
  if (stageId === "conceptCheck") {
    return fallback.slice(0, Math.min(5, fallback.length));
  }
  ...
  if (stageId === "topic-test") {
    return fallback.slice(0, Math.min(10, fallback.length));
  }
...
export default function TopicLearning() {
  ...
  const { studyState, completeStage, addTopicNote, addTopicMistake } =
    useStudy();

  const [extendedProgress, setExtendedProgress] =
    useState(readLearningProgress);

  const [assessmentResult, setAssessmentResult] = useState(null);

  const topic = studyState.topics.find((item) => item.id === topicId);

  const stageIndex = LEARNING_STAGES.findIndex((item) => item.id === stage);
  ...
  const progress = useMemo(
    () => (topic ? calculateTopicProgress(topic, extendedProgress) : 0),
    [topic, extendedProgress],
  );

  const assessmentKey = JSON.stringify([stage, topicId]);
  const [previousAssessmentKey, setPreviousAssessmentKey] =
    useState(assessmentKey);
  ...
  if (!topic) {
    return (
      <div className="mx-auto max-w-3xl rounded-lg border border-zinc-800 bg-zinc-900 p-5 text-center">
        <h1 className="text-2xl font-bold">Topic not found</h1>

        <button
          type="button"
          onClick={() => navigate("/syllabus")}
          className="mt-5 rounded-xl bg-cyan-900 px-5 py-3"
        >
          Return to Syllabus
        </button>
      </div>
  ...
  if (!currentStage) {
    return <div className="p-5 text-white">Invalid stage: {stage}</div>;
  }

  const status = getStageStatus(topic, stage, extendedProgress);

  const isCompleted = status === LEARNING_STATUS.COMPLETED;
  ...
  const openStage = (stageItem, index) => {
    const nextStatus = getStageStatus(topic, stageItem.id, extendedProgress);

    if (nextStatus !== LEARNING_STATUS.LOCKED) {
      navigate(`/topic/${topic.id}/${LEARNING_STAGES[index].id}`);
    }
  };
  ...
  const finishStage = () => {
    ...
    const nextProgress = markStageComplete(topic.id, stage, extendedProgress);
    ...
    if (nextStage) {
      navigate(`/topic/${topic.id}/${nextStage.id}`);
    } else {
      navigate("/syllabus");
    }
  };
  ...
  const stageDescriptions = {
    ...
    "level-1": "Build accuracy with straightforward foundation questions.",
    "level-2": "Apply the concept to mixed and moderately difficult questions.",
    "level-3": "Solve advanced SSC-level variations and strengthen speed.",
    "topic-test":
  ...
  return (
    <div className="mx-auto max-w-7xl pb-12">
      ...
      {status === LEARNING_STATUS.LOCKED ? (
        <div className="mt-6 rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-5 text-center">
          <h2 className="mt-4 text-2xl font-bold">Stage locked</h2>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_330px]">
          <section>
            {stage === "learn" && <LearnContent content={content} />}
            ...
            {stage === "revision" && <RevisionContent content={content} />}
            <StageFooter

...
const STATUS_STYLES = {
  approved: "bg-cyan-300/10 text-cyan-200",
  rejected: "bg-cyan-300/10 text-cyan-200",
  pending: "bg-cyan-300/10 text-cyan-200",
};

const fieldClass =
  "w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2.5 text-sm text-white outline-none transition focus:border-cyan-300/40";

export default function ReviewCenter() {
  const { questions, updateQuestion, setBulkStatus, saveState } =
    useReviewCenter();
  ...
  const stats = useMemo(
    () => ({
      ...
      approved: questions.filter((item) => item.reviewStatus === "approved")
        .length,
      pending: questions.filter((item) => item.reviewStatus === "pending")
        .length,
      rejected: questions.filter((item) => item.reviewStatus === "rejected")
        .length,
    }),
    [questions],
  );
  ...
  if (previousQuestions !== questions) {
    ...
    setSelected((ids) =>
      ids.filter((id) => questions.some((item) => item.id === id)),
    );
  }
  ...
  function patch(updates) {
    if (
      ...
      !canApproveQuestion({ ...current, ...updates })
    ) {
      window.alert(
        "Add the question, four options, answer key, subject and topic before approval.",
      );
      return;
    }
  ...
  function runBulk(nextStatus) {
    ...
    if (
      ...
      questions.some(
        (question) =>
          selected.includes(question.id) && !canApproveQuestion(question),
      )
    ) {
      window.alert(
        "Some selected questions are incomplete. Review their question, options, answer key and classification first.",
      );
      return;
    }
  ...
  const topicOptions = current ? SUBJECT_TOPICS[current.subject] || [] : [];
    ...
    <div className="mx-auto flex min-h-full max-w-[1800px] flex-col gap-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold text-white">Review Center</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-cyan-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-cyan-900 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Export JSON
          </button>
        </div>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          ["Approved", stats.approved, "text-cyan-200"],
          ["Pending", stats.pending, "text-cyan-200"],
          ["Rejected", stats.rejected, "text-cyan-200"],
      ...
      <section className="grid min-h-[420px] flex-1 overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900/80 xl:grid-cols-[390px_minmax(0,1fr)]">
        <aside className="flex min-h-0 flex-col border-b border-zinc-800 xl:border-b-0 xl:border-r">
          <div className="space-y-3 border-b border-zinc-800 p-4">
            <div className="relative">
              <input
                ...
                aria-label="Search questions, topics, sources"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="rounded-lg bg-cyan-900 px-3 py-2 text-xs font-semibold disabled:opacity-40"
              >
                Approve
              </button>
              <button
          ...
          <div className="max-h-[480px] flex-1 space-y-2 overflow-y-auto p-3">
            {filtered.length ? (
              filtered.map((item, index) => (
                ...
                >
                  <input
                    ...
                    aria-label="Select question for batch review"
                  />
                  ...
                  >
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center justify-between gap-2">
                        ...
                        <span
                          className={`rounded-lg px-2 py-1 text-[10px] font-bold capitalize ${STATUS_STYLES[item.reviewStatus]}`}
                        >
                          {item.reviewStatus}
                        </span>
                      </span>
          ...
          {current ? (
            <div className="mx-auto max-w-4xl space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label>
                  <span className="mb-2 flex justify-between text-sm font-semibold text-zinc-300">
                    ...
                    <span className="text-cyan-200">{current.confidence}%</span>
                  </span>
                  <input
                    type="range"
                    min="0"
              ...
              <div className="flex flex-wrap justify-end gap-3 border-t border-zinc-800 pt-5">
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-cyan-900 px-4 py-2.5 text-sm font-semibold"
                >
                  Pending
                </button>
                <button
            ...
            <div className="flex h-full min-h-[420px] flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 max-w-sm text-sm text-zinc-500">
                Extracted questions will appear here automatically after the
                Gemini Extractor saves its JSON output.
              </p>
            </div>
          )}
        </section>

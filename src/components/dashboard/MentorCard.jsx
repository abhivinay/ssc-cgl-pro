...
export default function MentorCard() {
  ...
  const activeStage = studyState.mission?.stage || "learn";
  ...
  if (brainCompleted && revisionDue.length) {
    ...
    instruction = `Proceed with ${subjectNames[missionTopic.subject] || missionTopic.subject} and complete the ${stageNames[activeStage] || activeStage} stage.`;
    recommendation =
      "Complete each stage in order and analyze mistakes before unlocking the next step.";
    actionLabel = "Continue Mission";
    actionRoute = `/topic/${missionTopic.id}/${routeStageIds[activeStage] || activeStage}`;
      ...
      <div className="relative p-6 md:p-5">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              <Badge variant="primary">Sentinel Mentor</Badge>

              <Badge variant={statusVariant}>{status}</Badge>
            </div>

            <p className="mt-6 text-xs font-bold uppercase tracking-[0.22em] text-cyan-200">
              {eyebrow}
            </p>

            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white md:text-3xl">
              {title}
            </h2>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <InsightItem
            label="Active Stage"
            value={
              missionTopic ? stageNames[activeStage] || activeStage : "None"
            }
            accent="violet"
          />
        </div>

        <div className="mt-6 rounded-lg border border-white/10 bg-black/20 px-5 py-4">
          <p className="text-xs leading-6 text-zinc-500">
            Required sequence: Learn → Concept Check → Level 1 → Level 2 → Level
            3 → Topic Test → PYQ → Revision.
          </p>
        </div>
      </div>
    </GlassCard>
...
function InsightItem({ label, value, accent = "violet" }) {
  const accents = {

...
export default function TestActions({ onChange }) {
  const [message, setMessage] = useState("");

  const runAction = (item) => {
    if (
      item.confirm &&
      !window.confirm("This will clear all SSC Sentinel local data. Continue?")
    ) {
      return;
    }
            ...
            {group.actions.map((item) => (
              <button
                ...
                className={`rounded-xl px-4 py-2.5 text-sm font-semibold transition ${
                  item.danger
                    ? "border border-cyan-300/30 bg-cyan-300/10 text-cyan-200 hover:bg-cyan-300/10"

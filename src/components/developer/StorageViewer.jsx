...
export default function StorageViewer({ refreshKey = 0, onChange }) {
  ...
  const handleImport = (event) => {
    ...
    reader.onload = () => {
      try {
        ...
        console.error(error);
        setFileError(error?.message || "Invalid backup file.");
      }
    };
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap gap-3">
        <button
          ...
          className="rounded-xl bg-cyan-900 px-4 py-2.5 text-sm font-semibold transition hover:bg-cyan-900"
        >
          Export JSON
        </button>

        <label className="cursor-pointer rounded-xl bg-cyan-900 px-4 py-2.5 text-sm font-semibold transition hover:bg-cyan-900">
          Import JSON
          <input
            type="file"
      ...
      {fileError && (
        <div className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm text-cyan-200">
          {fileError}
        </div>
      )}
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-semibold">Local Storage Snapshot</p>

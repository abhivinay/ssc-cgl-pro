...
const getTopicName = (content) =>
  content?.name || content?.learn?.introduction?.title || "Topic";

export default function StageRenderer({ stage, content, onStageComplete }) {
  if (!content) {
    return (
  ...
  switch (stage) {
    case "learn":
      if (content.structuredLearn)
        return <AuthoredLesson content={content.learn} />;
      if (primaryLesson) {
        return (
          <LessonPlayer
      ...
      return <LearnStage content={content.learn} />;
    ...
    case "revision":
      if (content.revision && !content.revision.summary)
        return (
          <AuthoredRevision

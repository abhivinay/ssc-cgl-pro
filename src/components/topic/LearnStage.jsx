...
export default function LearnStage({ content }) {
  ...
  const objectives = baseObjectives.length ? baseObjectives : unitObjectives;
  ...
  const standardConversions = toArray(content?.standardConversions);
  ...
  const commonMistakes = toArray(content?.commonMistakes);
      ...
      <div className="space-y-6">
        <GlassCard padding="p-0" className="relative overflow-hidden">
          <div className="relative p-6 md:p-5">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-3xl">
                <div className="flex flex-wrap items-center gap-3">
                  <Badge variant="primary">Learn Stage</Badge>
                ...
                <h2 className="mt-5 text-3xl font-semibold tracking-tight text-white md:text-3xl">
                  {activeUnit?.title || introduction?.title || "Topic Overview"}
                </h2>

                <p className="mt-4 text-base leading-8 text-zinc-300">
                  {activeUnit?.what || introduction?.definition}
                </p>
            ...
            {activeUnit && <UnitOverview unit={activeUnit} />}
          </div>
        </GlassCard>

        {objectives.length > 0 && (
          <SectionCard badge="Learning Objectives" title="What You Will Master">
        ...
        {sections.map((section, index) => (
          ...
          >
            ...
            {section.rule && (
              <InfoBox label="Rule" value={section.rule} accent="violet" />
            )}
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {standardConversions.map((item, index) => (
                ...
                >
                  <p className="font-semibold text-white">{item.fraction}</p>
          ...
          >
            <div className="grid gap-3 md:grid-cols-2">
              {sscPatterns.map((item, index) => (
                <div
                  key={`${item}-${index}`}
                  className="rounded-lg border border-cyan-300/15 bg-cyan-300/10 p-4"
                >
...
function UnitOverview({ unit }) {
  ...
  if (!phases.length && !majorSites.length && !how.length) {
    return null;
  }
  ...
  return (
    <div className="mt-7 grid gap-4 lg:grid-cols-3">
      {phases.length > 0 && (
        <OverviewCard title={unit.when?.label || "When"}>
          <div className="space-y-3">
            {phases.map((phase, index) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-zinc-500">{phase.period}</p>
              </div>
            ))}
          </div>
        </OverviewCard>
        ...
        <OverviewCard title="Where">
          ...
          <div className="flex flex-wrap gap-2">
            {majorSites.map((site) => (
              <span
                key={site}
                className="rounded-lg border border-cyan-300/15 bg-cyan-300/10 px-3 py-1 text-xs font-bold text-cyan-200"
              >
                {site}
              </span>
            ))}
      ...
      {how.length > 0 && (
        <OverviewCard title="How It Worked">
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      {items.map((item) => (
        ...
        >
          <p
            className={`text-xs font-semibold uppercase tracking-[0.16em] ${getAccentText(
              item.accent,
            )}`}
          >
            {item.label}
...
function ContentList({ title, items, accent = "violet" }) {
  const safeItems = toArray(items);

  if (!safeItems.length) return null;

  const styles = {
    violet: "border-cyan-300/15 bg-cyan-300/10",
    cyan: "border-cyan-400/15 bg-cyan-500/[0.04]",
    amber: "border-cyan-300/15 bg-cyan-300/10",
    rose: "border-cyan-300/15 bg-cyan-300/10",
  };

  return (
    <div className="mt-6">
      <p
        className={`text-xs font-semibold uppercase tracking-[0.18em] ${getAccentText(
          accent,
        )}`}
      >
        {title}
      ...
      <div className="mt-3 space-y-3">
        {safeItems.map((item, index) => (
          ...
          >
            <span className={getAccentText(accent)}></span>
...
function VisualCard({ visual, onOpen }) {
  ...
  const examFocus = toArray(visual.examFocus || visual.examImportancePoints);

  return (
    <div className="overflow-hidden rounded-lg border border-cyan-400/15 bg-cyan-500/[0.035]">
...
function VisualModal({ visual, onClose }) {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl overflow-hidden rounded-lg border border-white/10 bg-zinc-950 ">
        ...
        <div className="p-5 md:p-5">
          {visual.caption && (
            <p className="text-sm leading-7 text-zinc-300">{visual.caption}</p>
          )}
    ...
    <div className="mt-6 space-y-4">
      ...
      {examples.map((example, index) => (
        <div
          ...
          className="rounded-lg border border-cyan-400/15 bg-cyan-500/[0.04] p-5"
        >
          <p className="text-sm font-semibold text-white">{example.question}</p>
...
function InfoBox({ label, value, accent = "violet" }) {
  const styles = {
    violet: "border-cyan-300/20 bg-cyan-300/10 text-cyan-200",
    emerald: "border-cyan-300/20 bg-cyan-300/10 text-cyan-200",
    amber: "border-cyan-300/20 bg-cyan-300/10 text-cyan-200",
    sky: "border-cyan-300/20 bg-cyan-300/10 text-cyan-200",
    rose: "border-cyan-300/20 bg-cyan-300/10 text-cyan-200",
  };

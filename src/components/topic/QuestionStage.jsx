...
export default function QuestionStage({
  ...
  useEffect(() => {
    try {
      ...
      /* A global recovery notice retains the current in-memory attempt. */
    }
  }, [
    cacheKey,
    currentIndex,
  ...
  const submitAnswer = () => {
    if (selectedAnswer === null || showExplanation || !currentQuestion) {
      return;
    }

    const correctAnswer = getSafeAnswerIndex(currentQuestion);
  ...
  const nextQuestion = () => {
    ...
    if (isLastQuestion) {
      ...
      const finalCorrect = answers.filter((item) => item.correct).length;

      const finalAccuracy = finalAnswersLength
        ? Math.round((finalCorrect / finalAnswersLength) * 100)
        : 0;
      ...
      onComplete?.({
        ...
        wrongAnswers: finalAnswersLength - finalCorrect,
        totalAttempts: finalAnswersLength,
        accuracy: finalAccuracy,
        passed: finalAccuracy >= passingPercentage,
      });
    ...
    return (
      <GlassCard padding="p-0" className="relative overflow-hidden">
        <div className="relative p-6 text-center md:p-5">
          <div
            className={`mx-auto flex h-24 w-24 items-center justify-center rounded-lg border text-3xl ${
              passed
          ...
          <Badge variant={passed ? "success" : "danger"} className="mt-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <ResultCard label="Wrong" value={wrongAnswers} accent="rose" />
      ...
      <GlassCard padding="p-0" className="relative overflow-hidden">
        <div className="relative p-6 md:p-5">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            ...
            <MiniMetric label="Wrong" value={wrongAnswers} accent="rose" />
        ...
        <div className="relative p-6 md:p-5">
          ...
          <div className="mt-6 space-y-3">
            {currentQuestion.options.map((option, index) => {
              ...
              const correctIndex = getSafeAnswerIndex(currentQuestion);

              const revealCorrect = showExplanation && index === correctIndex;

              const revealWrong =
                showExplanation && selected && index !== correctIndex;

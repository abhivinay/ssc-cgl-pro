  ...
  return (
    <div className="space-y-5">
        ...
        {revisionPoints.length ? (
          revisionPoints.map((item, index) => (
            <div
              ...
              className="flex gap-4 rounded-lg border border-zinc-800 bg-zinc-900 p-5"
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-cyan-300/10 font-bold text-cyan-200">
                {index + 1}
              </span>

              <p className="leading-7 text-zinc-300">
                {getText(item, "Revision point is not available.")}
              </p>
            </div>
          ))
        ) : (
          <div className="rounded-lg border border-dashed border-zinc-800 bg-zinc-900/50 p-5 text-center">
      ...
      <div className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-6">
        <h3 className="font-bold text-cyan-200">Final revision rule</h3>

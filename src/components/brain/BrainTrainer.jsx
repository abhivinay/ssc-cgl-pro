...
export default function BrainTrainer() {
  ...
  const [initial] = useState(() => {
    try {
      ...
      if (saved?.date === getToday() && Array.isArray(saved.puzzles))
        return saved;
    } catch {
      /* Ignore malformed legacy cache when initializing the session. */
    }
    return {
      puzzles: [...PUZZLES].sort(() => Math.random() - 0.5).slice(0, 5),
      index: 0,
      score: 0,
      timeLeft: 600,
      finished: false,
  ...
  const [timeLeft, setTimeLeft] = useState(
    Math.max(
      0,
      Number.isFinite(Number(initial.timeLeft))
        ? Number(initial.timeLeft)
        : 600,
    ),
  );
  ...
  const finishSession = useCallback(
    (finalScore) => {
      ...
      if (safeScore > best)
        localStorage.setItem("brainBest", String(safeScore));
  ...
  if (daily.length === 0)
    return <div className="card">Loading Brain Trainer...</div>;
  ...
  return (
    <div className="card">
      ...
      {finished ? (
        ...
        <>
          <p>
            Puzzle {index + 1} / {daily.length}
          </p>
          <div
            style={{ fontSize: "1.1rem", fontWeight: 600, margin: "16px 0" }}
          >
            {daily[index].q}
          </div>
          <input
